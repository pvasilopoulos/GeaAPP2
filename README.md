...
- **Ownership hierarchy.** Each customer owns its branches (`branches.customer_id`); each branch owns its spaces (`spaces.branch_id`). Customers have **multiple contacts**, live **tags**, photo upload, and duplicate warnings (email/phone/ΑΦΜ). Branches have **status, opening hours, manager, map coordinates**. Spaces have **status, amenities, booking rules, and tiered pricing**.
