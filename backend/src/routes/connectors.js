...
connectorsRouter.post('/', guard, async (req, res, next) => {
  try {
    ...
    const result = await query(`INSERT INTO connectors (tenant_id,name,base_url,method,auth_type,credentials_enc,body_template,headers,mappings,schedule_minutes,enabled,timeout_ms,retry_count) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)`,
      [req.user.tenantId, b.name.trim(), b.base_url, b.method || 'GET', b.auth_type || 'bearer',
...
connectorsRouter.patch('/:id', guard, async (req, res, next) => {
  try {
    ...
    const fields = ['name','base_url','method','auth_type','body_template','headers','mappings','schedule_minutes','enabled','timeout_ms','retry_count']; const sets = [], params = [];
