...
CREATE TABLE customers (
  ...
  last_name        VARCHAR(120) NOT NULL,
  company          VARCHAR(200),
  full_name        VARCHAR(255) GENERATED ALWAYS AS (CASE WHEN company IS NOT NULL AND TRIM(company) <> '' THEN company ELSE TRIM(CONCAT(first_name, ' ', last_name)) END) STORED,
  email            VARCHAR(255),
  phone            VARCHAR(40),
  mobile           VARCHAR(40),
  tax_id           VARCHAR(40),
