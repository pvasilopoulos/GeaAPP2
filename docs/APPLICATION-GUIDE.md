...
- **Περιεχόμενο**: τίτλος, κείμενο, προεπιλεγμένος σύνδεσμος κλικ.
- **Πολυμέσα**: μεγάλη εικόνα μέσα στην ειδοποίηση (`image`), αντικατάσταση εικονιδίου/badge ανά
  αποστολή (αλλιώς χρησιμοποιούνται τα branding assets του §3.2).
- **Κουμπιά ενεργειών**: έως 2 κουμπιά, καθένα με δικό του σύνδεσμο — στο κλικ πάνω σε κουμπί
  ανοίγει το δικό του URL αντί του προεπιλεγμένου.
- **Συμπεριφορά**: `requireInteraction` (παραμένει έως αλληλεπίδραση), `silent` (χωρίς
  ήχο/δόνηση), `tag` (ομαδοποίηση/αντικατάσταση προηγούμενης ειδοποίησης με το ίδιο tag),
  `renotify` (ξανά-ειδοποίηση/ήχος όταν αντικαθίσταται μια ειδοποίηση με ίδιο tag), προσαρμοσμένο
  μοτίβο δόνησης (presets ή ελεύθερο `ms,ms,…`).
- **Προτεραιότητα παράδοσης**: `urgency` (`very-low`…`high`, στέλνεται στον push service ως Web
  Push option — επηρεάζει πόσο «επιθετικά» ξυπνά τη συσκευή) και `TTL` (πόσο θα προσπαθήσει ο
  push service να παραδώσει σε offline συσκευή πριν το πετάξει, 1 λεπτό–28 ημέρες).
- **Στόχευση**: όλοι οι χρήστες, ένας συγκεκριμένος ρόλος (`GET /api/push/roles`), ή λίστα
  συγκεκριμένων χρηστών με αναζήτηση. Η ίδια η λίστα παραληπτών **επαναϋπολογίζεται τη στιγμή της
  αποστολής** (`resolveRecipients()`) — σημαντικό για προγραμματισμένες αποστολές, ώστε να
  φτάνουν σε όποιον είναι ενεργός τότε και όχι σε ένα «παγωμένο» στιγμιότυπο τη στιγμή σύνθεσης.
- **Προγραμματισμός**: αποστολή τώρα ή σε μελλοντική ημ/ωρα (`sendAt`). Αν είναι πάνω από 60″
  στο μέλλον αποθηκεύεται ως `pending` και παραδίδεται από το sweep του scheduler
  (`sendScheduledBroadcasts()`, τρέχει στο ίδιο 60″ tick με τα υπόλοιπα background jobs). Οι
  προγραμματισμένες αποστολές μπορούν να ακυρωθούν πριν την ώρα τους (`POST
  /api/push/broadcasts/:id/cancel`).
- **Πρότυπα (Templates)**: αποθήκευση/φόρτωση/διαγραφή έτοιμων συνόλων πεδίων με όνομα
  (`push_templates` table, `GET/POST/DELETE /api/push/templates` — το `POST` κάνει upsert στο
  ίδιο όνομα).
- **Click tracking**: κάθε αποστολή μετρά πόσες φορές πατήθηκε (`clicked_count`) μέσω ενός public
  beacon (`POST /api/push/click`, χωρίς αυθεντικοποίηση επίτηδες — το service worker δεν έχει JWT).
- **Ιστορικό**: κατάσταση (`sent`/`pending`/`cancelled`/`failed`), αριθμός παραληπτών, πόσα push
  όντως στάλθηκαν, πόσα κλικ, αποστολέας, ώρα.

Τεχνικά: το `frontend/public/sw.js` χτίζει το πλήρες `NotificationOptions` object από το push
payload (image/actions/requireInteraction/silent/vibrate/tag/renotify) και στο
`notificationclick` είτε ανοίγει το URL του πατημένου κουμπιού είτε κάνει deep-link στο
προεπιλεγμένο target — σε κάθε περίπτωση στέλνει πρώτα το click beacon. Στο backend, το
`sanitizeRichPush()` καθαρίζει/κλειδώνει όλα τα πεδία (π.χ. actions ≤ 2, vibrate 0–5000ms/όρο,
...
- **Γεγονός (event)**: `follow_up_overdue`, `follow_up_due_soon`, `follow_up_assigned`,
  ...
  μεταβλητών template ανά γεγονός) εκτίθεται από `GET /api/notification-rules/events`.
- **Συνθήκες (AND)**: λίστα `{ πεδίο, τελεστής, τιμή }` — τελεστές `=`, `≠`, `>`, `≥`, `<`, `≤`,
  «περιέχει». Άδεια λίστα = ο κανόνας πυροδοτείται πάντα. Π.χ. «`total` ≥ 5000» για να
  ειδοποιείται ένας manager μόνο για μεγάλες προσφορές.
- **Παραλήπτες**: όλοι οι χρήστες, συγκεκριμένος ρόλος, συγκεκριμένη λίστα χρηστών, ή **δυναμικός
...
- **Throttle**: προαιρετικό ελάχιστο διάστημα (σε δευτ.) μεταξύ δύο διαδοχικών πυροδοτήσεων του
  ...
  γεγονότα που μπορεί να επαναληφθούν γρήγορα (π.χ. πολλαπλές αλλαγές κατάστασης προσφοράς).
- **Ενεργός/ανενεργός** διακόπτης ανά κανόνα, και μετρητής πόσες φορές έχει πυροδοτηθεί.

**Ρυθμίσεις → Ειδοποιήσεις → «Οι ειδοποιήσεις μου»** (κάθε χρήστης, χωρίς ειδικό δικαίωμα):
πίνακας γεγονός × κανάλι με checkboxes — προεπιλεγμένα όλα ενεργά (**opt-out**, όχι opt-in), ώστε
ένας νέος κανόνας να φτάνει αμέσως σε όποιον στοχεύει μέχρι να τον απενεργοποιήσει ο ίδιος. Στο
...
προϋπάρχοντες κανόνες event-based συνεχίζουν να δουλεύουν χωρίς καμία αλλαγή
(`trigger_type = 'event'` είναι η προεπιλογή).

- **Τύπος κανόνα** (`triggerType`):
  - `event` — όπως παραπάνω (κάτι μόλις συνέβη).
  - `schedule` — «Χ λεπτά/ώρες/ημέρες πριν/μετά από το πεδίο ημερομηνίας Υ της οντότητας Ζ».
    Ο κατάλογος οντοτήτων/πεδίων ημερομηνίας/μεταβλητών εκτίθεται από
    `GET /api/notification-rules/schedule-entities` (`lib/scheduleEntities.js`) και καλύπτει
    **Προσφορά** (λήξη ισχύος ή δημιουργία), **Ραντεβού/Κράτηση** (ώρα έναρξης), **Follow-up**
    (προθεσμία) και **Πελάτης** (δημιουργία ή τελευταία δραστηριότητα). Παράδειγμα: «Προσφορά →
    πεδίο "Ημερομηνία λήξης ισχύος" → 3 ημέρες πριν → ειδοποίησε τον πωλητή» καλύπτει ακριβώς το
    σενάριο «η προσφορά κοντεύει να λήξει». Η επανάληψη μπορεί να είναι **μία φορά** (ανά
    εγγραφή, για πάντα) ή **καθημερινή** (όσο η εγγραφή παραμένει «ζωντανή» — π.χ. καθημερινή
    υπενθύμιση όσο μια προσφορά είναι ακόμα σε εκκρεμότητα). Ο σαρωτής `sweepScheduleTriggers()`
    τρέχει στο ίδιο 60″ tick με τα υπόλοιπα sweeps και αποτρέπει διπλή πυροδότηση μέσω του πίνακα
    `notification_schedule_fired`.
- **Ομάδες συνθηκών (AND/OR)**: πέρα από μία απλή λίστα συνθηκών (ΚΑΙ μεταξύ τους), μπορείς να
  φτιάξεις πολλαπλές **ομάδες** συνθηκών και να τις συνδυάσεις με **Ή** — π.χ. «(status = 'sent'
  ΚΑΙ total ≥ 5000) Ή (status = 'sent' ΚΑΙ customerName περιέχει 'VIP')».
- **Επιπλέον ενέργειες (`extraActions`)** εκτελούνται αυτόματα κάθε φορά που πυροδοτείται ο
  κανόνας, πέρα από την ίδια την ειδοποίηση:
  - **Δημιουργία follow-up** (τίτλος + προθεσμία σε Χ ημέρες) στον πελάτη του context.
  - **Προσθήκη ετικέτας (tag)** στον πελάτη.
  - **Κλήση webhook** (POST σε εξωτερικό URL με το context ως JSON body) — χρήσιμο για
    ενσωμάτωση με άλλα εργαλεία (Zapier/Make/δικό σου endpoint).
- **Κλιμάκωση (escalation)**: αν κανείς από τους αρχικούς παραλήπτες δεν έχει διαβάσει την
  ειδοποίηση μέσα σε Χ λεπτά, στέλνεται αυτόματα και σε επιπλέον παραλήπτες (π.χ. στον manager)
  μέσω `sweepEscalations()`.
- **Ομαδοποίηση (digest mode)**: αντί άμεσης αποστολής, οι ειδοποιήσεις μπαίνουν σε ουρά
  (`notification_digest_queue`) και στέλνονται μαζεμένα **ανά ώρα** ή **1 φορά/ημέρα (08:00)** —
  χρήσιμο για κανόνες χαμηλής προτεραιότητας που δεν χρειάζονται άμεση ειδοποίηση.
- **Προτεραιότητα**: `normal` ή `urgent` — τα `urgent` αγνοούν πάντα τις ώρες ησυχίας.
- **Ώρες ησυχίας**: κάθε χρήστης ορίζει το δικό του διάστημα (`quiet_hours_start/end`) στο
  **Ρυθμίσεις → Ειδοποιήσεις → «Οι ειδοποιήσεις μου»**· ένας κανόνας που έχει ενεργό «Σεβασμός
  ωρών ησυχίας» δεν θα στείλει `app`/`sms`/`viber`/`telegram` (το `email` εξαιρείται ως μη
  ενοχλητικό) μέσα σε αυτό το διάστημα, εκτός αν είναι `urgent`.
- **Δοκιμαστική λειτουργία (dry-run)**: όταν είναι ενεργή, ο κανόνας «πυροδοτείται» κανονικά
  (καταγράφεται στο execution log) αλλά **δεν** στέλνει καμία πραγματική ειδοποίηση — χρήσιμο για
  να δοκιμάσεις νέους κανόνες με ασφάλεια πριν τους ενεργοποιήσεις πλήρως.
- **«Δοκιμή τώρα»**: κουμπί στον επεξεργαστή κανόνα (`POST /api/notification-rules/:id/test`) που
  τρέχει τον κανόνα άμεσα σε dry-run πάνω σε ένα πραγματικό δείγμα εγγραφής, ώστε να ελέγξεις
  templates/συνθήκες χωρίς να περιμένεις τον σαρωτή.
- **Πρόσφατες εκτελέσεις**: κάθε πυροδότηση (πραγματική ή dry-run) καταγράφεται στο
  `notification_rule_runs` και εμφανίζεται στον επεξεργαστή κανόνα
  (`GET /api/notification-rules/:id/runs`).

Όλα τα παραπάνω μοιράζονται την ίδια εσωτερική συνάρτηση `dispatchRule(rule, context, queryFn)`
(`lib/notificationRules.js`), οπότε παραλήπτες/κανάλια/templates/throttle/προτιμήσεις δουλεύουν
πανομοιότυπα είτε ο κανόνας είναι event-based είτε schedule-based.
